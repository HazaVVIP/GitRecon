...
For a pinned release or a manual checksum-verified download, use the assets listed on the [v3.2.8 release page](https://github.com/HazaVVIP/GitRecon/releases/tag/v3.2.8). The Linux archive is named `gitrecon-3.2.8-linux-x86_64.tar.gz`; verify it against the matching entry in `SHA256SUMS` before extraction.
...
Reports are written below the selected output directory. The default JSON report contains detection metadata, mapping information, findings, severity counts, risk score, scan statistics, technology fingerprints, object-source distribution, cache hit/miss and cache lifecycle telemetry, rate-limit counters, optional HTTP retry telemetry (`result.retry`), file-save counters, and typed skip/failure outcomes. Retry telemetry includes wire-level `attempts`, `retries`, successful and failed outcomes, network errors, and observed status counts; it is populated for URL scans and built-in HTTP-backed forge clients, while local scans and custom Forge implementations may return `null`. The same additive operational telemetry is included in token-mode JSON reports. For bounded archive scans, `result.outcomes.archive_truncated` records the number of archive/decompression/depth limit events; `result.outcomes.archive_invalid_reasons` distinguishes malformed, traversal, depth, entry, expansion, GZIP-limit, and unsupported-format states. Resource telemetry is additive: `result.outcomes.resource_by_stage` reports `current_bytes`, `peak_bytes`, and `denied_reservations` for the stable acquisition, object_scan, archive, workspace_reconstruction, file_scan, and target_fanout stages; URL acquisition, object scanning, ZIP/GZIP archive expansion, forge workspace reconstruction, and forge workspace file scanning are instrumented, while stages not yet holding a reservation in a given scan mode remain zero rather than being presented as active. Workspace reservations cover the declared or configured per-blob upper bound through fetch and workspace materialization; file-scan reservations begin before each file read and live through classification and scanning. Archive-stage budget denials remain visible in `archive_invalid_reasons.resource_budget` and the typed `skipped_resource_budget` count. URL scans also expose `result.outcomes.scheduler` with acquire/grant counts, queue count and wait milliseconds, active peak, current limit, limit adjustments, and adaptive adjustment/throttle/headroom events. Zero means no such event was observed, not that malformed input was forcefully expanded. When `--resume` restores a stream checkpoint, monotonic resource and scheduler counters are carried forward so final telemetry includes the completed history; in-flight bytes are not restored and correctly begin at zero in the new process.

For multi-target runs with `--parallel-targets > 1`, `aggregate_report.json` additionally includes `target_fanout` telemetry with `configured_limit`, `current_active`, `peak_active`, `current_waiters`, `queue_wait_count`, `queue_wait_ms`, `permits_granted`, `targets_started`, and `targets_completed`. This is control-plane permit telemetry for the actual target execution gate; it does not pretend that a target has a fixed byte footprint, so `resource_by_stage.target_fanout` remains zero until a genuine byte-allocation boundary is instrumented.

The non-JSON formats preserve their existing finding consumers while adding a metadata envelope. SARIF stores the envelope under `runs[0].properties.gitrecon`; NDJSON emits one reserved first record with `_gitrecon.record_type=metadata`; CSV keeps its original header and rows and writes telemetry to the adjacent `<report>.meta.json` sidecar; Markdown renders a `Scan metadata` section; and HTML renders escaped telemetry JSON in a `<pre>` section. These envelopes contain operational counts and scope only, never token material or finding matches. `--save` writes reconstructed source under a target-specific directory. Treat reports and reconstructed source as sensitive because findings may contain plaintext credential material.
...
The repository includes unit tests, deterministic object-source acquisition contracts using local TCP response fixtures, forge identity/retrieval/forbidden/error contracts, path-aware Bitbucket contracts, pagination regression tests for GitHub, GitLab, and Bitbucket, checkpoint-directory resume coverage, CLI help coverage, and binary-level integration tests using temporary fixtures. Tests do not require live credentials or external network access. The remote acquisition benchmark can be run with `python3 tools/benchmark_remote_acquisition.py --binary target/release/gitrecon`; it emits JSON from a localhost-only Git fixture with per-sample throughput, peak RSS, cache/retry data, scheduler queue pressure, and resource-budget telemetry plus aggregate descriptive statistics. It is observational and intentionally does not impose naive wall-clock thresholds.

See [DEVELOPMENT.md](DEVELOPMENT.md) for production defaults, maintenance rules, architecture details, and the release checklist. See [docs/limitations.md](docs/limitations.md) for the capability matrix and explicit URL, local, forge snapshot, forge history, binary/archive, cache, and platform limitations.
