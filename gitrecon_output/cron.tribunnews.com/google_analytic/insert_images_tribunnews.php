...
$analytics = $google_analytic->getReportTopImagesTribunnews();
...
if($analytics['status']){
	...
	if(count($dataPopuler) > 0){
		...
		foreach($dataPopuler as $val){
			...
			$fields = array("title", "date_upload", "views_count");
			$response = $opensearch->findOne("tribunnews-images", $query, $fields);
			//$response = $opensearchTest->findOne("tribunnews-images", $query, $fields);
			...
			if($response['status']){
				...
				$responseUpdate = $opensearch->updateOne("tribunnews-images", $id, $arrUpdate);
				//$responseUpdate = $opensearchTest->updateOne("tribunnews-images", $id, $arrUpdate);
