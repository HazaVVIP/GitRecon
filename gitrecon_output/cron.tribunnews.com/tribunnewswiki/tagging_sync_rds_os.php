...
if(count($arrID) > 0){
	foreach($arrID as $id){
		$sqlRow = "SELECT a.subtitle, c.id as tagging_id, c.title as tagging_title, c.alias as tagging_alias
		FROM articles a
		...
		if(count($arrTaging) > 0){
			...
			$dataUpdateOS['subtitle'] = $subtitle;
			$dataUpdateOS['subtitle_alias'] = $subtitle_alias;
			
			$responseUpdateOs = $opensearch->updateOne('tribunnewswiki-articles',$id,$dataUpdateOS);
