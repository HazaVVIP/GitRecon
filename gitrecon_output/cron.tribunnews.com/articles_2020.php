...
$writelog->Log(PATH_ROOT."/logs/","tribunnews-articles-2020");

$lastid = 0;
$total = 0;
$filename = PATH_ROOT . "/data/lastid_articles_2020.txt";
...
if (empty($lastid)) {
	...
	$start = 0;
	$limit = 2250;
	$response = $elasticsearch->find('tribunnews-articles-2020',$condition,$fields,$sort,$start,$limit);
	
	$response_total = $elasticsearch->count_total('tribunnews-articles-2020',$condition);
} else {
...
if($response['status']){
	...
	if($totalData > 0){
		foreach($arrPosts as $idx => $post){
			...
			$responseInsert = $opensearch->insert("tribunnews-articles-2020", $arrInsert);
