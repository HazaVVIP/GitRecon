	...
	if($totalOs != $totalEs){
		...
		if(count($arrID) > 0){
			foreach($arrID as $sid){
				...
				$response_one = $opensearch->findOne("tribunnews-video", $condition);
				...
				if($response_one['status']){
					...
					$responseInsert = $elasticsearch->insert("tribunnews-video", $arrInsert);
...
echo "Total SYNC Video OS ke ES : ".$totalSyncEs."<br>";
