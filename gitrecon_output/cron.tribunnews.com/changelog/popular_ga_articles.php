...
$writelog->Log(PATH_ROOT."/logs/","tribunnews-populer-ga-articles");

$lastid = 0;
$total = 0;
$filename = PATH_ROOT . "/data/lastid_populargaarticles.txt";
...
if($response['status']){
	...
	if($totalData > 0){
		foreach($arrPosts as $idx => $post){
			...
			$foto_type 			= isset($post['_source']['foto_type'])?$post['_source']['foto_type']:0;
			$foto_name 			= isset($post['_source']['foto_name'])?$post['_source']['foto_name']:0;
			$written_date 		= isset($post['_source']['written_date'])?$post['_source']['written_date']:"";
			$introtext 			= isset($post['_source']['introtext'])?$post['_source']['introtext']:0;
			...
			$responseInsert = $opensearch->insert("tribunnews-populer-ga-articles", $arrInsert);
