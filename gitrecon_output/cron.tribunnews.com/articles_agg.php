...
$writelog->Log(PATH_ROOT."/logs/","tribunnews-articles-agg");

$lastid = 0;
$total = 0;
$filename = PATH_ROOT . "/data/lastid_articles_agg.txt";
...
if (empty($lastid)) {
	...
	$start = 0;
	$limit = 1000;
	$response = $elasticsearch->find('tribunnews-articles_agg',$condition,$fields,$sort,$start,$limit);
	
	$response_total = $elasticsearch->count_total('tribunnews-articles_agg',$condition);
} else {
...
if($response['status']){
	...
	if($totalData > 0){
		foreach($arrPosts as $idx => $post){
			...
			$alias_old 					= isset($post['_source']['alias_old'])?$post['_source']['alias_old']:"";
			...
			$editor_video_by 			= isset($post['_source']['editor_video_by'])?$post['_source']['editor_video_by']:"";
			...
			$writter 					= isset($post['_source']['writter'])?intval($post['_source']['writter']):0;
			...
			//if(empty($source_group)){
				$sqlSource = "SELECT name_source, url_source, source_group FROM source_news WHERE id = ".$source;
				...
				$source_group  = isset($rowSource['source_group'])?$rowSource['source_group']:$source_group;
			//}
			
			$sqlUsersEditor = "SELECT username, fullname FROM users WHERE id = ".$editor_id;
			...
			$sqlUsersWritter = "SELECT username, fullname FROM users WHERE id = ".$writter_id;
			...
			$responseInsert = $opensearch->insert("tribunnews-articles_agg", $arrInsert);
