...
- sudo -u www-data /usr/bin/php7.4 /var/www/html/web-cron/daerah/daerah_cluster_new_articles_sync/daerah_cluster_new_articles_sync.php bekasi
*/
	...
	if($response['status']){
		...
		if($totalData > 0){
			foreach($arrPosts as $idx => $post){
				...
				$modified_date 				= isset($post['_source']['modified_date'])?$post['_source']['modified_date']:null;
