...
/* 
Running in command
- sudo -u cron /usr/bin/php7.4 /var/www/html/web-cron/daerah/daerah_sync_os_allnetwork.php aceh
*/
...
if(!empty($daerah)){
	...
	//RDS
	$daerahDB = $daerah;
	$daerahDB = str_replace("aceh","aceh2",$daerahDB);
	$daerahDB = str_replace("jambi","jambi2",$daerahDB);
	$con = mysqli_connect(RDS_DAERAH_HOST,RDS_DAERAH_USERNAME,RDS_DAERAH_PASSWORD,$daerahDB);
	...
	if(count($arrID) > 0){
		...
		foreach($arrID as $id){
			...
			if(!empty($id)){
				...
				if(!mb_check_encoding($subtitle_alias, 'UTF-8')){
					$subtitle_alias = mb_convert_encoding ($subtitle_alias, 'UTF-8');
					$subtitle_alias = str_replace("?"," ",$subtitle_alias);
				}
				...
				if(isset($arrInsert['title']) && !empty($arrInsert['title'])){
					...
					array_push($whereAllNetwork,array("match_phrase" => array("domain" => $daerah)));
					...
					if(!$responseCheck['status']){
						...
						$arrInsertAllNetwork['domain'] = $daerah;
