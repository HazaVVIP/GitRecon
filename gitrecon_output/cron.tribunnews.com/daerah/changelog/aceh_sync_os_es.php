...
if(!empty($daerah)){
	...
	$daerahRds = str_replace("aceh","aceh2",$daerah);
	$daerahRds = str_replace("jambi","jambi2",$daerahRds);
	$con = mysqli_connect(RDS_DAERAH_HOST,RDS_DAERAH_USERNAME,RDS_DAERAH_PASSWORD,$daerahRds);
	...
	//if(count($arrIDEs) > 0 && count($arrIDOs) > 0){
		//if($totalEs != $totalOs){
			$arrID = array_diff($arrIDOs, $arrIDEs);
			...
			if(count($arrID) > 0){
				foreach($arrID as $sid){
					...
					if($response_one['status']){
						...
						$arrInsert['frontpage_section'] = boolval($frontpage_section);
						$arrInsert['frontpage_category'] = boolval($frontpage_category);
						...
						$responseInsertES = $elasticsearch->insert($index, $arrInsert);
...
echo "Total SYNC OS ke ES : ".$totalSyncEs."<br>";
