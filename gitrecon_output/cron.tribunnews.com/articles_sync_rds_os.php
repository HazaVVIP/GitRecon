...
if(count($arrID) > 0){
	$idx = 1;
	foreach($arrID as $id){
		$sqlRow = "SELECT a.id, a.title, a.alias, a.alias_old, a.subtitle, a.keyword, a.foto_type, a.foto_name, a.foto_caption, a.foto_source,
		...
		$responseInsertOs = $opensearch->insert("tribunnews-articles", $arrInsert);
		...
		if($responseInsertOs['status']){
			...
			if(!$responseCheck['status']){
				...
				if(!empty($allnetwork_id)){
					...
					$responseUpdateOs = $opensearch->updateOne("tribunnews-articles",$id,$dataUpdateOS);
				}
			}	
		}
	}
