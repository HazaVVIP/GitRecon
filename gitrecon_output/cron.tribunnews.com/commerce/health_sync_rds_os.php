...
- sudo -u cron /usr/bin/php7.4 /var/www/html/web-cron/tbo/health_sync_rds_os.php
*/
...
$site = "health";
$date = isset($_SERVER["argv"][1])?$_SERVER["argv"][1]:"";
if(isset($_GET['date'])){
	$date = $_GET['date'];
}

if(!empty($site) && in_array($site, LIST_COMMERCE_CLUSTER)){
	if(!empty($date)){
		$dateStart = $date;
		$dateEnd = $date;
	} else {	
	...
	if(count($arrID) > 0){
		...
		foreach($arrID as $id){
			if($site == "shopping"){
				...
				a.editor_by, a.written_by, a.written_date, a.publish_date, a.modified_date, a.source, a.youtube, a.kategori, a.affiliate,
				...
				$sqlRow = "SELECT a.id, a.title, a.alias, a.subtitle, a.foto_type, a.foto_name, a.foto_caption, a.foto_source,
				a.introtext, a.fulltexts, a.section_id, a.category_id, a.publish, a.modified_date, a.frontpage_section, a.frontpage_category,
			...
			if(!empty($id)){
				...
				$subtitle_alias 			= !empty($subtitle)?str_replace(" ","_",strtolower($subtitle)):"";
				...
				if($site == "shopping"){
					...
					$affiliate 					= isset($post['affiliate'])?intval($post['affiliate']):0;
				}
				$arrFotoName 				= explode("/",$foto_name);
				$foto_cross_domain 			= isset($arrFotoName[1])?1:0;
				$modified_date 				= isset($post['modified_date'])?$post['modified_date']:"";
				$id_narasumber 				= isset($post['id_narasumber'])?intval($post['id_narasumber']):0;
				...
				$sqlRow = "SELECT c.id as tagging_id, c.title as tagging_title, c.alias as tagging_alias, a.id_narasumber
				FROM articles a
				...
				while($post = mysqli_fetch_array($resultRow, MYSQLI_ASSOC))
				{
					...
					$id_narasumber = intval($post['id_narasumber']);
				}
				...
				$title_narasumber = "";
				$url_narasumber = "";
				$description_narasumber = "";
				$image_narasumber = "";
				$sosmed_account_narasumber = "";
				$status_narasumber = "";
				if(!empty($id_narasumber)){
					$sqlNaraSumber = "SELECT type_source, value_source, title_source, url_source, description_source, image_source, sosmed_account, status 
					FROM narasumber WHERE id = ".$id_narasumber;
					$resultNaraSumber = mysqli_query($con, $sqlNaraSumber);
					$rowNaraSumber = mysqli_fetch_array($resultNaraSumber, MYSQLI_ASSOC);
					$type_narasumber  = isset($rowNaraSumber['type_source'])?$rowNaraSumber['type_source']:"";
					$value_narasumber  = isset($rowNaraSumber['value_source'])?$rowNaraSumber['value_source']:"";
					$title_narasumber  = isset($rowNaraSumber['title_source'])?$rowNaraSumber['title_source']:"";
					$url_narasumber  = isset($rowNaraSumber['url_source'])?$rowNaraSumber['url_source']:"";
					$description_narasumber  = isset($rowNaraSumber['description_source'])?$rowNaraSumber['description_source']:"";
					$image_narasumber  = isset($rowNaraSumber['image_source'])?$rowNaraSumber['image_source']:"";
					$sosmed_account_narasumber  = isset($rowNaraSumber['sosmed_account'])?$rowNaraSumber['sosmed_account']:"";
					$status_narasumber  = isset($rowNaraSumber['status'])?intval($rowNaraSumber['status']):0;
				}
				...
				$arrInsert['id_narasumber'] = $id_narasumber;
				$arrInsert['type_narasumber'] = $type_narasumber;
				$arrInsert['value_narasumber'] = $value_narasumber;
				$arrInsert['title_narasumber'] = $title_narasumber;
				$arrInsert['url_narasumber'] = $url_narasumber;
				$arrInsert['description_narasumber'] = $description_narasumber;
				$arrInsert['image_narasumber'] = $image_narasumber;
				$arrInsert['sosmed_account_narasumber'] = $sosmed_account_narasumber;
				$arrInsert['status_narasumber'] = $status_narasumber;
				...
				if($responseInsertOs['status']){
					...
					if(!$responseCheck['status']){
						...
						if(!empty($allnetwork_id)){
							...
							$dataUpdateOS['title'] = $title;
							$dataUpdateOS['introtext'] = $introtext;
							$dataUpdateOS['fulltexts'] = $fulltexts;
