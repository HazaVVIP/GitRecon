...
- sudo -u cron /usr/bin/php7.4 /var/www/html/web-cron/commerce/health_migrasi_articles.php
*/
	...
	if($response['status']){
		...
		if($totalData > 0){
			foreach($arrPosts as $idx => $post){
				...
				$sqlRow = "SELECT c.id as tagging_id, c.title as tagging_title, c.alias as tagging_alias, a.id_narasumber, a.written_date, a.publish_date
				FROM articles a
				...
				if($responseInsertOs['status']){
					...
					if(!$responseCheck['status']){
						...
						if(!empty($allnetwork_id)){
							...
							if(!empty($id_narasumber)){
								$dataUpdateOS['id_narasumber'] = $id_narasumber;
								$dataUpdateOS['type_narasumber'] = $type_narasumber;
								$dataUpdateOS['value_narasumber'] = $value_narasumber;
								$dataUpdateOS['title_narasumber'] = $title_narasumber;
								$dataUpdateOS['url_narasumber'] = $url_narasumber;
								$dataUpdateOS['description_narasumber'] = $description_narasumber;
								$dataUpdateOS['image_narasumber'] = $image_narasumber;
								$dataUpdateOS['sosmed_account_narasumber'] = $sosmed_account_narasumber;
								$dataUpdateOS['status_narasumber'] = $status_narasumber;
							}	
