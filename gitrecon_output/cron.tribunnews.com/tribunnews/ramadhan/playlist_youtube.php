<?php
ini_set('display_errors',1);
error_reporting(E_ALL);
ini_set("memory_limit", "-1");
set_time_limit(0);

$time_start = time();

define("DOC_ROOT","/var/www/html/web-cron/");

include DOC_ROOT."config/config.php";
include DOC_ROOT."lib/Opensearch.php";

$playlist_name = isset($_SERVER["argv"][1])?$_SERVER["argv"][1]:"";
if(isset($_GET['playlist_name'])){
	$playlist_name = $_GET['playlist_name'];
}	

$arrPlaylist = array(

					array("playlist_name"=> "ramadhan-lebaran-2026", "playlist_id"=> "PLRUmxnvpvtsva0_Xs3BUF2FLWMcjhilk9"),
					array("playlist_name"=> "sedap-mantap", "playlist_id"=> "PLRUmxnvpvtssA2mW3nPthsKDeCiMpUyNQ"),
					array("playlist_name"=> "segar-mantap", "playlist_id"=> "PLRUmxnvpvtsthR-268JffWfzTmGzV7FAV"),
					array("playlist_name"=> "mutiara-ramadhan", "playlist_id"=> "PLRUmxnvpvtstwFdJAQLPFyn8iRIs4b-Ec"),
					array("playlist_name"=> "mutiara-ramadhan-2026", "playlist_id"=> "PLRUmxnvpvtss62BOLUcyjwVHiRaeTzxBa"),
					array("playlist_name"=> "niat-doa", "playlist_id"=> "PLRUmxnvpvtssRjunyC2KFjV3B_VGJONqW"),
					array("playlist_name"=> "tanya-ustaz", "playlist_id"=> "PLRUmxnvpvtstcXegeShuPeRP-2csIdhHY"),
					array("playlist_name"=> "jadwal-imsakiyah", "playlist_id"=> "PLRUmxnvpvtssJR1nmHEbygIykdlWoVdkf"),
					array("playlist_name"=> "ngabuburit-asyik", "playlist_id"=> "PLRUmxnvpvtstBc_fJmjAagfOeUORM2_pN"),
					array("playlist_name"=> "cerita-para-kiai", "playlist_id"=> "PLRUmxnvpvtsui2cK3Inem6wm0Sl_1z01G"),
					array("playlist_name"=> "ramadhan-cantik", "playlist_id"=> "PLRUmxnvpvtsvA2pIFccIr3b8qYKwUmc_T"),
					array("playlist_name"=> "tanya-ustaz-rumahzakat", "playlist_id"=> "PLRUmxnvpvtsuLlVxTlxBhQF1COoYx32I0"),
				);
				

$arrayKey = searcharray($playlist_name, "playlist_name", $arrPlaylist);

$videoList = array();

$channelId = 'UCmxAIW7RDDC88EPk4ry16Kg';
$apiKey = 'AIzaSyCZHyglyvDMfdXrlc6oUYzZnTdMrMBEh5I';
$max = 20;

$playlistId = isset($arrPlaylist[$arrayKey]['playlist_id'])?$arrPlaylist[$arrayKey]['playlist_id']:"";

if(!empty($playlistId)){
	$apiv3_getPlaylist = 'https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&channelId='.$channelId.'&key='.$apiKey.'&playlistId='.$playlistId.'&maxResults='.$max.'&order=viewcount&type=video';
	
	$apierror = 'Not found';
	try{
		$apiData = @file_get_contents($apiv3_getPlaylist);
		
		if($apiData){
			$jsonVideoList = json_decode($apiData, TRUE);
			
			$videoList = isset($jsonVideoList['items'])?$jsonVideoList['items']:array();
		}else{
			throw new Exception('Invalid API Key or Channel ID');
		}
	}catch(Exception $e){
		$apierror = $e->getMessage();
	}
}	


//print_r($videoList);

$arrRamadhan = array();
if(count($videoList) > 0){
	//OS
	//$opensearchDev = new Opensearch();
	//$opensearchDev->init(OS_DEV_URL,OS_DEV_USERNAME,OS_DEV_PASSWORD,true);
	
	$opensearch = new Opensearch();
	$opensearch->init(OS_TBO_URL,OS_TBO_USERNAME,OS_TBO_PASSWORD,true);
	
	foreach($videoList as $k => $video){
		if(@$video['snippet']['title'] == 'Private video'){
			continue;
		}

		$id = $k+1;
		$title = @$video['snippet']['title'];
		$title = utf8_encode($title);
		$title = preg_replace('/[^(\x20-\x7F)]*/','', $title);
		$title = str_replace("'","\'",$title);
		$title = trim($title);
		$description = @$video['snippet']['description'];
		$description = html_entity_decode(str_replace("'","\'",$description), ENT_QUOTES, "UTF-8");
		$published_at = date("Y-m-d H:i:s",strtotime((string)$video['snippet']['publishedAt']));
		$youtube_video_id = @$video['snippet']['resourceId']['videoId'];
		
		$arrRamadhan[$k]['id'] = $arrPlaylist[$arrayKey]['playlist_name']."-".$id;
		$arrRamadhan[$k]['youtube_video_id'] = $youtube_video_id;
		$arrRamadhan[$k]['title'] = $title;
		$arrRamadhan[$k]['alias'] = url_title($title);
		$arrRamadhan[$k]['description'] = $description;
		$arrRamadhan[$k]['published_at'] = $published_at;
		$arrRamadhan[$k]['playlist_id'] = $arrPlaylist[$arrayKey]['playlist_id'];
		$arrRamadhan[$k]['playlist_name'] = ucwords(str_replace(array("-"),array(" "),$arrPlaylist[$arrayKey]['playlist_name']));
		$arrRamadhan[$k]['playlist_alias'] = $arrPlaylist[$arrayKey]['playlist_name'];
		$arrRamadhan[$k]['create_date'] = date("Y-m-d H:i:s");
		
		$arrInsert = array();
		$arrInsert['id'] = $arrPlaylist[$arrayKey]['playlist_name']."-".$id;
		$arrInsert['youtube_video_id'] = $youtube_video_id;
		$arrInsert['title'] = $title;
		$arrInsert['alias'] = url_title($title);
		$arrInsert['description'] = $description;
		$arrInsert['published_at'] = $published_at;
		$arrInsert['playlist_id'] = $arrPlaylist[$arrayKey]['playlist_id'];
		$arrInsert['playlist_name'] = ucwords(str_replace(array("-"),array(" "),$arrPlaylist[$arrayKey]['playlist_name']));
		$arrInsert['playlist_alias'] = $arrPlaylist[$arrayKey]['playlist_name'];
		$arrInsert['create_date'] = date("Y-m-d H:i:s");
		
		//$responseInsertOs = $opensearchDev->insert("playlist_youtube_ramadan", $arrInsert);
		
		$responseInsertOs = $opensearch->insert("playlist_youtube_ramadan", $arrInsert);
		
		echo "<pre>";
		print_r($responseInsertOs);
		print_r($arrInsert);
		echo "</pre>";
	}	
	
	//unset($opensearchDev);
	unset($opensearch);
}


/* echo "<pre>";
print_r($arrRamadhan);
echo "</pre>"; */

function searcharray($value, $key, $array) {
	foreach ($array as $k => $val) {
		if ($val[$key] == $value) {
		   return $k;
		}
	}
	return null;
}

function url_title($str, $separator = '-', $lowercase = TRUE)
{
	if ($separator == 'dash') 
	{
		$separator = '-';
	}
	else if ($separator == 'underscore')
	{
		$separator = '_';
	}
	
	$q_separator = preg_quote($separator);

	$trans = array(
		'&.+?;'                 => '',
		'[^a-z0-9 _-]'          => '',
		'\s+'                   => $separator,
		'('.$q_separator.')+'   => $separator
	);

	$str = strip_tags($str);

	foreach ($trans as $key => $val)
	{
		$str = preg_replace("#".$key."#i", $val, $str);
	}

	if ($lowercase === TRUE)
	{
		$str = strtolower($str);
	}

	return trim($str, $separator);
}

echo '<br>Execution time in seconds: ' . (microtime(true) - $time_start) . "<br>";
?>
