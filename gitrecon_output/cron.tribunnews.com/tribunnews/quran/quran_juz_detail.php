...
$juz = isset($_SERVER["argv"][1]) ? intval($_SERVER["argv"][1]) : 0;
if (isset($_GET['juz'])) {
    $juz = intval($_GET['juz']);
}
...
if (!empty($juz)) {
    $urlJson = "https://asset-2.tribunnews.com/tribunnews/alquran/json/juz/".$juz.".json";
	...
	if(is_countable($arrAlQuranCache) && count($arrAlQuranCache) > 0){
		$juzInfo = $arrAlQuranCache['juz'] ?? array();
		$ayahList = $arrAlQuranCache['juz']['ayah'] ?? array();
	} else {
		...
		$response = $opensearchTBO->find('quran', $condition, $fields, $sort, 0, 700);

		if (isset($response['status']) && $response['status'] == 1 && !empty($response['data'])) {
			$noAyat = 1;
			$firstRow = isset($response['data'][0]['_source']) ? $response['data'][0]['_source'] : array();
			$lastRow = end($response['data']);
			$lastRow = isset($lastRow['_source']) ? $lastRow['_source'] : array();

			$juzInfo = array(
				'juz' => $juz,
				'total_ayat' => count($response['data']),
				'first_surah' => isset($firstRow['chapter_name_latin_id']) ? $firstRow['chapter_name_latin_id'] : '',
				'first_surah_id' => isset($firstRow['chapter_id']) ? $firstRow['chapter_id'] : '',
				'first_verse_key' => isset($firstRow['verse_key']) ? $firstRow['verse_key'] : '',
				'last_surah' => isset($lastRow['chapter_name_latin_id']) ? $lastRow['chapter_name_latin_id'] : '',
				'last_surah_id' => isset($lastRow['chapter_id']) ? $lastRow['chapter_id'] : '',
				'last_verse_key' => isset($lastRow['verse_key']) ? $lastRow['verse_key'] : ''
			);
			...
			foreach ($response['data'] as $row) {
				...
				$ayahList[] = array(
					...
					'chapter_alias'          => isset($src['chapter_alias']) ? $src['chapter_alias'] : '',
...
<?php if(count($ayahList) > 0){ ?>
	...
	<head>
		...
		<title><?php echo !empty($juzInfo) ? 'Juz ' . htmlspecialchars($juzInfo['juz']) . ' - Al-Qur\'an Digital' : 'Juz Detail - Al-Qur\'an Digital'; ?></title>
		<style>
			* {
	...
	<body>
		<div class="container">
			<a href="quran_juz.php" class="back-link">&larr; Kembali ke daftar juz</a>
			...
			<?php else: ?>
				<div class="juz-header">
					<h1 class="juz-title">Juz <?php echo htmlspecialchars($juzInfo['juz']); ?></h1>
					<div class="juz-subtitle">
						Total <?php echo htmlspecialchars($juzInfo['total_ayat']); ?> ayat<br>
						Awal: Surah <?php echo htmlspecialchars($juzInfo['first_surah']); ?> (<?php echo htmlspecialchars($juzInfo['first_verse_key']); ?>)<br>
						Akhir: Surah <?php echo htmlspecialchars($juzInfo['last_surah']); ?> (<?php echo htmlspecialchars($juzInfo['last_verse_key']); ?>)
					</div>
				</div>

				<?php
				$juzHlsUrl = "https://asset-2.tribunnews.com/tribunnews/alquran/hls/juz/" . $juzInfo['juz'] . "/juz.m3u8";
				?>
				<div class="player-box">
					<div class="player-label">Play Full Juz <?php echo htmlspecialchars($juzInfo['juz']); ?></div>
					<audio id="juz-player"
						   class="juz-player"
						   data-hls="<?php echo htmlspecialchars($juzHlsUrl); ?>"
						   controls
						   preload="none"></audio>
				</div>

				<?php foreach ($ayahList as $index => $ayah): ?>
					<div class="ayah-item" id="ayah-<?php echo $index; ?>" data-ayah-index="<?php echo $index; ?>">
						<div class="ayah-top">
							<div class="ayah-top-left">
								<div class="ayah-no-badge">
									<?php echo htmlspecialchars($ayah['no_ayat']); ?>
								</div>

								<div class="ayah-surah">
									<?php echo htmlspecialchars($ayah['chapter_name_latin_id']); ?>
									<?php if (!empty($ayah['verse_key'])): ?>
										- <?php echo htmlspecialchars($ayah['verse_key']); ?>
									<?php endif; ?>
								</div>
							</div>

							<div class="ayah-top-right">
								<span class="juz-badge">Juz <?php echo htmlspecialchars($ayah['juz']); ?></span>
							</div>
						</div>
						...
						<?php if (!empty($ayah['ayah_tafsir_short_id']) || !empty($ayah['ayah_tafsir_long_id'])): ?>
							<div class="tafsir-wrapper">
								...
								<div class="ayah-tafsir tafsir-hidden" id="tafsir-<?php echo $index; ?>">
									<?php if (!empty($ayah['ayah_tafsir_short_id'])): ?>
										<?php echo nl2br(htmlspecialchars($ayah['ayah_tafsir_short_id'])); ?>
									<?php endif; ?>

									<?php if (!empty($ayah['ayah_tafsir_long_id'])): ?>
										<?php if (!empty($ayah['ayah_tafsir_short_id'])): ?>
											<br><br><hr><br>
										<?php endif; ?>
						...
						<?php if (!empty($ayah['verse_key'])): ?>
							...
							$url_mp3 = "https://asset-2.tribunnews.com/tribunnews/alquran/mp3/" . $ayah['chapter_id'] . "/" . $verse_key . ".mp3";
							$url_hls = "https://asset-2.tribunnews.com/tribunnews/alquran/hls/surah/" . $ayah['chapter_id'] . "/" . $verse_key . "/index.m3u8";
							?>
		...
		document.addEventListener('DOMContentLoaded', function () {
			...
			const juzPlayer = document.getElementById('juz-player');
			...
			function stopJuzPlayer() {
				if (!juzPlayer) return;
				clearTimer(juzPlayer);
				juzPlayer.pause();
				juzPlayer.currentTime = 0;
				juzPlayer.dataset.shouldPlay = 'false';
			}
			...
			function initJuzPlayer(player) {
				...
				player.addEventListener('error', function () {
					console.log('Juz HLS gagal diputar');
				});

				if (player._hls) {
					player._hls.on(Hls.Events.ERROR, function (event, data) {
						if (data && data.fatal) {
							console.log('Juz HLS fatal error', data);
						}
					});
				}
			}

			initJuzPlayer(juzPlayer);
