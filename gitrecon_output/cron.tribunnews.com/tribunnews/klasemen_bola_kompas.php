...
if (!empty($liga)) {
	...
	if($liga == "inggris") die("Stop Kompas Not Update");
	if($liga == "prancis") die('STOP Kompas Not Update');
	if($liga == "bundesliga") die('STOP Kompas Not Update');
	if($liga == "indonesia") die('STOP Kompas Not Update');
	
	$html = file_get_html('https://bola.kompas.com/klasemen/'.$path_liga, false, null, 0);
	if($html != false){
		...
		for ($i=0; $i < count($datadiv); $i++) {
			...
			if(count($bodyData) > 0){
				for ($j=0; $j < count($bodyData); $j++) {
					...
					if (isset($bodyData[$j]['div'][1]['div'][0]['img']['@attributes']['src'])){
						$pattern = '/[\w\-]+\.(jpg|png|gif|jpeg)/';
						$subject = $bodyData[$j]['div'][1]['div'][0]['img']['@attributes']['src'];
						$result = preg_match($pattern, $subject, $matches);
						if ($bodyData[$j]['div'][1]['div'][1] == 'Mönchengladbach') $matches[0] = 'monchengladbach.png';
						if ($bodyData[$j]['div'][1]['div'][1] == 'Köln') $matches[0] = 'koln.png';
						if ($bodyData[$j]['div'][1]['div'][1] == 'Atlético Madrid') $matches[0] = 'atletico-madrid.png';
						if ($bodyData[$j]['div'][1]['div'][1] == 'Almería') $matches[0] = 'almeria.png';
						if ($bodyData[$j]['div'][1]['div'][1] == 'Cádiz') $matches[0] = 'cadiz.png';
						if ($bodyData[$j]['div'][1]['div'][1] == 'Viktoria Plzeň') $matches[0] = 'viktoria-plzen.png';
						if ($bodyData[$j]['div'][1]['div'][1] == 'Alavés') $matches[0] = 'alaves.png';
						$datagroup[$j]['image_klub_link'] = "https://t-1.tstatic.net/img/klubimageasset/".$liga."/".$matches[0];
					}else{
						$datagroup[$j]['image_klub_link'] = $defaultimage;
					}
