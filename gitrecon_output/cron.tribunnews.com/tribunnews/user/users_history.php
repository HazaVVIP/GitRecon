...
if(!empty($userId)){
	...
	$dateHistory = date("Ymd", strtotime('-2 days'));
	...
	if(count($rsArticleUserHistory) > 0){
		...
		if (!empty($topSections)) {
			...
			foreach($topSections as $key_sec => $sections){
				if(isset($sections['percent'])){
					if($sections['percent'] == 100){
					...
					else if($sections['percent'] >= 40 && $subdomain === "tribunnews"){
						array_push($arrMustSection,$sections['name']);
						...
						if($sections['name'] == "news") array_push($arrMustSection,"nasional");
						if($sections['name'] == "superskor") array_push($arrMustSection,"superball");
						unset($topSections[$key_sec]);
						$isMinShouldMatch = false;
					} else if($sections['percent'] < 10 && $subdomain === "tribunnews"){
						unset($topSections[$key_sec]);
					}	
				}
			}	
		...
		$arrBool = array();
		if(count($arrMust) > 0){
			$arrBool['must'] = $arrMust;
		}	
		if(count($arrShould) > 0){
			$arrBool['should'] = $arrShould;
		}	
		if(count($arrMustNot) > 0){
			$arrBool['must_not'] = $arrMustNot;
		}
		if($isMinShouldMatch){
			$arrBool['minimum_should_match'] = 1;
		}	
		$query = array(
					"bool" => $arrBool
				 );	
		...
		//$sort = array("_score" => array("order" => "desc"), "publish_date" => array("order" => "desc"));
		$sort = array("publish_date" => array("order" => "desc"), "pageviews" => array("order" => "asc"));
