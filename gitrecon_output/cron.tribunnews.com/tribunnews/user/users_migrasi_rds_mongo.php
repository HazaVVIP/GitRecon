...
if($totalRds > 0){
	while($post = mysqli_fetch_assoc($result))
	{
		...
		if(!empty($email)){
			...
			$fields = array("_id","old_id","username","email","kmps_usrid");
			...
			if(!$rsUserCheck){
				...
				$kmps_usrid_exist = isset($rsUserCheck['kmps_usrid'])?$rsUserCheck['kmps_usrid']:"";
				
				$data_doubleuser = $id_exist." = ".$old_id." - ".$username_exist." - ".$email_exist."\n";
				$writelog_doubleuser->doLogInfo($data_doubleuser);
				
				if(!empty($email_exist) && !empty($old_id_exist) && empty($kmps_usrid_exist)){
					...
					if($respond !== FALSE){
						...
						if($status_profile){
							$phone = isset($respond['profile']['phone'])?$respond['profile']['phone']:"";
							$gender = isset($respond['profile']['gender'])?$respond['profile']['gender']:"";
							$kmps_usrid_val = isset($respond['profile']['old_mykompas_id'])?$respond['profile']['old_mykompas_id']:"";
							
							$gender_val = 2;
							if(!empty($gender)){
								if($gender == "l") $gender_val = 1;
							}
							
							if(!empty($kmps_usrid_val)){
								$arrUpdate = array();
								$arrUpdate['kmps_usrid'] = $kmps_usrid_val;
								...
								if(!empty($gender_val)){
									$arrUpdate['gender'] = $gender_val;
								}

								$condition = array("old_id" => intval($old_id_exist));
								$rsUpdateUser = $mongodb->update('users', $condition, array('$set'=>$arrUpdate));	
					...
					//echo $id_exist." = ".$old_id." - ".$kmps_usrid_val." - ".$username_exist." - ".$email_exist." - ".$gender_val." - ".$phone."<br>";
				} else {
					//echo $id_exist." = ".$old_id." - ".$kmps_usrid_exist." - ".$username_exist." - ".$email_exist."<br>";
				}
			}	
		}
...
if (!empty($lastid)) {
	...
	$loginfo = "TOTAL = ".$totalDataRDS." | TOTAL INSERT = ".$total." | TOTAL UPDATE = ".$totalupdate." | LAST ID = ".$lastid."\n";
