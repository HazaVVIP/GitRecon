...
- sudo -u cron /usr/bin/php7.4 /var/www/html/web-cron/tbo/tbo_migrasi_articles.php style
*/
...
if(!empty($site)){
	...
	$lastid = 0;
	$total = 0;
	$filename_cached = PATH_ROOT . "/data/tbo/".$site."_lastid_articles.txt";
	...
	$response_totalTBO = $opensearchTBO->count_total($index,$condition);
	...
	if (empty($lastid)) {
		...
		$fields = array();
		$response = $opensearch->find($index,$query,$fields,$sort,0,$limit);	
	} else {
		$arrMust = array();
			...
			foreach($arrPosts as $idx => $posts){
				...
				if(count($post) > 0){
					...
					if(!empty($id)){	
						...
						array_push($whereCheck,array("match_phrase" => array("id" => $id)));
						...
						$fieldsCheck = array("id","pageviews","allnetwork_id");
						$responseCheck = $opensearchTBO->findOne($index,$queryCheck,$fieldsCheck);
						...
						if(!$responseCheck['status']){
							...
							$foto_position 				= isset($post['foto_position'])?$post['foto_position']:"";
							...
							$foto_cross_domain 			= isset($post['foto_cross_domain'])?intval($post['foto_cross_domain']):0;
							...
							$allnetwork_id 				= isset($post['allnetwork_id'])?intval($post['allnetwork_id']):"";
							$id_konten_kreatif 			= isset($post['id_konten_kreatif'])?intval($post['id_konten_kreatif']):"";
							...
							$responseInsertOs = $opensearchTBO->insert($index, $arrInsert);
							...
							if(!empty($pageviews)){
								$dataUpdateOS['pageviews'] = $pageviews;
							}
							
							if(count($dataUpdateOS) > 0){
								$responseUpdateOs = $opensearchTBO->updateOne($index,$id,$dataUpdateOS);
							}	
						}
	...
	if (!empty($lastid)) {
		...
		$loginfo = "Total OS Seblat = ".$totalOs." | Total OS VPC TBO  = ".$totalOsTBO." | TOTAL MIGRASI = ".$totalOsTBO." | LAST ID = ".$lastid."\n";
