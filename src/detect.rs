...
const PROBES: &[(&str, Verifier, u32)] = &[
    (
        "HEAD",
        |b| {
            b.windows(9).any(|w| w == b"ref: refs")
                || (b.len() >= 40 && b[..40].iter().all(|&c| c.is_ascii_hexdigit()))
        },
        40,
    ),
...
async fn probe_one_path(
    ...
    let server = detect_server(client, base_url).await;
    let listing = if score >= MIN_CONFIDENCE {
        check_listing(client, &git_url).await
    } else {
        false
    };
...
pub async fn run(client: &HttpClient, base_url: &str, fuzz: bool) -> Option<DetectResult> {
    let base_url = base_url.trim_end_matches('/');

    if !fuzz {
        // Fast path: only check the default `.git` location
        return probe_one_path(client, base_url, ".git")
            .await
    ...
    for h in handles {
        if let Ok(Some(r)) = h.await {
            let better = best
                .as_ref()
                .is_none_or(|b: &DetectResult| r.confidence > b.confidence);
            if better {
...
mod tests {
    ...
    fn test_path_variants_contains_default() {
        assert!(
            PATH_VARIANTS.contains(&".git"),
            "Default .git path must be present"
        );
    }
    ...
    fn test_path_variants_contains_backup_paths() {
        assert!(
            PATH_VARIANTS.contains(&".git.bak"),
            ".git.bak must be in PATH_VARIANTS"
        );
        assert!(
            PATH_VARIANTS.contains(&".git.old"),
            ".git.old must be in PATH_VARIANTS"
        );
    }
    ...
    fn test_path_variants_contains_azure_devops_path() {
        assert!(
            PATH_VARIANTS.contains(&"_git"),
            "_git (Azure DevOps) must be in PATH_VARIANTS"
        );
    }
    ...
    fn test_path_variants_contains_dist_build() {
        assert!(
            PATH_VARIANTS.contains(&"dist/.git"),
            "dist/.git must be in PATH_VARIANTS"
        );
        assert!(
            PATH_VARIANTS.contains(&"build/.git"),
            "build/.git must be in PATH_VARIANTS"
        );
    }
    ...
    fn test_path_variants_contains_api_versioned() {
        assert!(
            PATH_VARIANTS.contains(&"api/v1/.git"),
            "api/v1/.git must be in PATH_VARIANTS"
        );
        assert!(
            PATH_VARIANTS.contains(&"api/v2/.git"),
            "api/v2/.git must be in PATH_VARIANTS"
        );
    }
    ...
    fn test_path_variants_contains_common_subdirs() {
        assert!(
            PATH_VARIANTS.contains(&"frontend/.git"),
            "frontend/.git must be in PATH_VARIANTS"
        );
        assert!(
            PATH_VARIANTS.contains(&"server/.git"),
            "server/.git must be in PATH_VARIANTS"
        );
        assert!(
            PATH_VARIANTS.contains(&"client/.git"),
            "client/.git must be in PATH_VARIANTS"
        );
        assert!(
            PATH_VARIANTS.contains(&"mobile/.git"),
            "mobile/.git must be in PATH_VARIANTS"
        );
        assert!(
            PATH_VARIANTS.contains(&"static/.git"),
            "static/.git must be in PATH_VARIANTS"
        );
        assert!(
            PATH_VARIANTS.contains(&"uploads/.git"),
            "uploads/.git must be in PATH_VARIANTS"
        );
        assert!(
            PATH_VARIANTS.contains(&"storage/.git"),
            "storage/.git must be in PATH_VARIANTS"
        );
        assert!(
            PATH_VARIANTS.contains(&"dashboard/.git"),
            "dashboard/.git must be in PATH_VARIANTS"
        );
    }
    ...
    fn test_total_weight_matches_probe_sum() {
        ...
        assert_eq!(
            ...
            "TOTAL_WEIGHT must equal sum of probe weights"
        );
    }
    ...
    fn test_packed_refs_verifier_accepts_valid_format() {
        ...
        let verifier = PROBES
            .iter()
            .find(|(p, _, _)| *p == "packed-refs")
            .map(|(_, v, _)| v)
            .unwrap();
        assert!(
            verifier(valid),
            "packed-refs verifier must accept a properly formatted file"
        );
    }
    ...
    fn test_packed_refs_verifier_rejects_plain_hex() {
        ...
        assert!(
            !verifier(not_packed),
            "packed-refs verifier must reject a file with only stray hex chars"
        );
    }
    ...
    fn test_logs_head_verifier_accepts_valid_format() {
        ...
        let valid = format!(
            "{} {} Author <a@b.com> 1234567890 +0000\tCommit\n",
            old_sha, new_sha
        );
        let verifier = PROBES
            .iter()
            .find(|(p, _, _)| *p == "logs/HEAD")
            .map(|(_, v, _)| v)
            .unwrap();
        assert!(
            verifier(valid.as_bytes()),
            "logs/HEAD verifier must accept a properly formatted log line"
        );
    }
    ...
    fn test_logs_head_verifier_rejects_plain_hex() {
        ...
        assert!(
            !verifier(not_log),
            "logs/HEAD verifier must reject a file with only stray hex chars"
        );
    }
    ...
    fn test_protected_confidence_is_above_min() {
        const {
            assert!(
                ...
                "PROTECTED_CONFIDENCE must be at least MIN_CONFIDENCE so protected paths pass"
            );
        }
    }
    ...
    fn test_path_variants_contains_framework_paths() {
        assert!(
            PATH_VARIANTS.contains(&"laravel/.git"),
            "laravel/.git should be in PATH_VARIANTS"
        );
        assert!(
            PATH_VARIANTS.contains(&"django/.git"),
            "django/.git should be in PATH_VARIANTS"
        );
        assert!(
            PATH_VARIANTS.contains(&"rails/.git"),
            "rails/.git should be in PATH_VARIANTS"
        );
    }
    ...
    fn test_path_variants_contains_env_paths() {
        assert!(
            PATH_VARIANTS.contains(&"staging/.git"),
            "staging/.git should be in PATH_VARIANTS"
        );
        assert!(
            PATH_VARIANTS.contains(&"demo/.git"),
            "demo/.git should be in PATH_VARIANTS"
        );
        assert!(
            PATH_VARIANTS.contains(&"sandbox/.git"),
            "sandbox/.git should be in PATH_VARIANTS"
        );
    }
    ...
    fn test_path_variants_contains_nested_paths() {
        assert!(
            PATH_VARIANTS.contains(&"packages/.git"),
            "packages/.git should be in PATH_VARIANTS"
        );
        assert!(
            PATH_VARIANTS.contains(&"services/.git"),
            "services/.git should be in PATH_VARIANTS"
        );
    }
}
